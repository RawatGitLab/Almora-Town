...
export default function App() {
  ...
  const toggleLayerGroup = (ids: string[], visible: boolean) => {
    setLayers((prev) => {
      if (!visible && selectedFeature) {
        ...
        const targetLayers = prev.filter((l) => idsSet.has(l.id));
        if (targetLayers.some((l) => l.name === featLayerName)) {
          setSelectedFeature(null);
        }
      }
      const idsSet = new Set(ids);
      return prev.map((l) => (idsSet.has(l.id) ? { ...l, visible } : l));
    });
  };
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-extrabold tracking-tight text-white uppercase">District Almora Town</span>
          ...
          <div className="absolute inset-x-0 inset-y-0 bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center z-50 p-6 select-none font-sans">
            <div className="bg-slate-800 border border-slate-700/80 p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-sm text-center">
              ...
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                Connecting securely to database. Downloading geographical boundaries, river streams, and villages of <span className="text-indigo-400 font-semibold">Almora&nbsp;Town</span>...
              </p>
